...
// Función para mostrar las ventas en el modal
const mostrarVentas = (ventasFiltradas) => {
    const modal = document.getElementById("modal_empleados");
    const listaProductos = document.getElementById("listaProductos");
    ...
    ventasFiltradas.forEach((venta) => {
        ...
        const idVenta = document.createElement("p");
        idVenta.textContent = `ID Venta: ${venta.id}`;

        const totalFactura = document.createElement("p");
        totalFactura.textContent = `Total Factura: ${venta.productos[0].total_factura}`;
        
        // Agregar otros detalles de la venta si es necesario
        li.appendChild(idVenta);
        li.appendChild(totalFactura);
...
const listar_historial = async () => {  
    ...
    detalles.forEach((e) => {
        e.addEventListener("click", () => {
            ...
            // Filtrar las ventas que corresponden al empleado
            const ventasFiltradas = ventas.filter(venta =>
                venta.productos.some(producto => producto.id_empleado === empleadoId)
            );
